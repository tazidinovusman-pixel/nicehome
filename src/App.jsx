...
import { Heart, ShoppingBag, User, Sun, Moon, ShieldCheck, Menu, X, Search, Sofa, UtensilsCrossed, BedDouble, Bath, Baby, DoorOpen, Laptop, TreePine, LayoutGrid } from 'lucide-react';
...
const translations = {
  KG: { shop: "Дүкөн", admin: "Админ", profile: "Профиль", login: "Кирүү", search: "Издөө...", categories: { all: "Баары", living: "Конок бөлмө", kitchen: "Ашкана", bedroom: "Уктоочу бөлмө", bathroom: "Ванна", kids: "Балдар бөлмөсү", hallway: "Кире бериш", office: "Иш бөлмө", garden: "Бакча" } },
  RU: { shop: "Магазин", admin: "Админ", profile: "Профиль", login: "Войти", search: "Поиск...", categories: { all: "Все", living: "Гостиная", kitchen: "Кухня", bedroom: "Спальня", bathroom: "Ванная", kids: "Детская", hallway: "Прихожая", office: "Кабинет", garden: "Сад" } },
  EN: { shop: "Store", admin: "Admin", profile: "Profile", login: "Login", search: "Search...", categories: { all: "All", living: "Living Room", kitchen: "Kitchen", bedroom: "Bedroom", bathroom: "Bathroom", kids: "Kids Room", hallway: "Hallway", office: "Office", garden: "Garden" } }
};
...
function App() {
  ...
  return (
    <LanguageContext.Provider value={{ user, lang, setLang, translations, darkMode, setDarkMode }}>
      <CartProvider>
        <Router>
          <div className={`min-h-screen flex flex-col transition-colors ${darkMode ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`}>
            
            {/* --- HEADER --- */}
            <nav className={`sticky top-0 z-[100] border-b backdrop-blur-xl ${darkMode ? 'bg-slate-950/90 border-slate-800' : 'bg-white/90 border-slate-50'}`}>
                ...
                <div className="pb-3 px-1 md:px-0">
                   <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl border transition-all ${darkMode ? 'bg-slate-900 border-slate-800 focus-within:border-indigo-500' : 'bg-slate-50 border-slate-100 focus-within:border-indigo-400'}`}>
                     ...
                     <input 
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            // Издөө Enter басылганда URL-ди өзгөртөт
            ...
            {isMenuOpen && (
              <div className="fixed inset-0 z-[110] flex">
                ...
                <div className={`relative w-[280px] h-full shadow-2xl overflow-y-auto flex flex-col ${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
                    ...
                    <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-800 rounded-2xl">
                      {['KG', 'RU', 'EN'].map((l) => (
                        <button
                          ...
                          className={`flex-1 py-2 rounded-xl font-bold text-[10px] transition-all ${lang === l ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400'}`}
                        >
                          {l}
                        </button>
                      ))}
                  ...
                  <div className="flex-grow p-4 space-y-1">
                    ...
                    {menuCategories.map((cat) => (
                      <Link 
                        key={cat.key}
                        // 'all' болсо башкы бетке, калганы параметр менен
                      ...
                      >
                        <span className="text-slate-400 group-hover:text-indigo-500">{cat.icon}</span>
